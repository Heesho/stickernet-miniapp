...
export function BaseAccountAuth({ onAuthenticated, className = "" }: BaseAccountAuthProps) {
      ...
      <Wallet>
        ...
        >
          ...
          <span className="text-white">
            {isConnected && basename ? basename : (isConnected && address ? `${address.slice(0, 6)}...${address.slice(-4)}` : 'Sign in with Base')}
          </span>
        </ConnectWallet>
            ...
            <div className="flex flex-col">
              <div className="flex items-center gap-1">
                <span className="text-base font-semibold">
                  {basename || (address ? `${address.slice(0, 6)}...${address.slice(-4)}` : 'Not connected')}
                </span>
                {basename && (
                  <Identity address={address || '0x0'}>
                    <Badge />
      ...
      {isConnected && (
        <div className="mt-4 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
          <div className="text-sm">
            ...
            {basename && (
              <div className="flex items-center justify-between mb-2">
                <span className="text-gray-600 dark:text-gray-400">Basename:</span>
