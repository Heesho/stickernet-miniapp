...
export function CurateConfirmation({ curate, nextPrice, onClose, onSuccess }: CurateConfirmationProps) {
  ...
  useEffect(() => {
    if (status.approveStatus === 'success' && status.curateStatus === 'success') {
  ...
  const hasEnoughBalance = balanceData && parseFloat(formattedBalance) >= parseFloat(nextPrice);
    ...
    <div className="fixed inset-0 bg-background z-50 flex justify-center">
      <div className="w-full max-w-md bg-black min-h-screen flex flex-col">
        ...
        <h1 className="text-white text-2xl font-bold mb-8">
          Steal {curate.token.symbol || curate.token.name || 'PENGU'} {curate.tokenId.toString()}
        </h1>
        ...
        {/* Transaction Status */}
        {(status.approveStatus !== 'idle' || status.curateStatus !== 'idle') && (
          <div className="mb-4 p-3 rounded-xl bg-gray-900">
            <div className="flex items-center justify-between">
              ...
              <span className={`text-xs px-2 py-1 rounded-full ${
                ...
                status.approveStatus === 'pending' || status.curateStatus === 'pending'
                  ? 'bg-yellow-500/20 text-yellow-400 animate-pulse' :
                status.approveStatus === 'error' || status.curateStatus === 'error'
