...
export function useChartData({ tokenAddress, timeframe, enabled = true }: UseChartDataProps): UseChartDataReturn {
  ...
  const fetchChartData = useCallback(async () => {
    ...
    const result = await errorHandler.executeWithErrorHandling(async () => {
      ...
      if (timeframe === 'LIVE' || timeframe === '4H') {
        ...
        const startHourIndex = Math.max(tokenCreatedHourIndex, currentHourIndex - (numHours - 1));
        ...
        const startDayIndex = Math.max(tokenCreatedDayIndex, currentDayIndex - 29); // Up to 30 days
  ...
  const process1MChartData = (
    ...
    for (let i = 0; i < totalDays; i++) {
      ...
      if (dayIndex < tokenCreatedDayIndex) {
        ...
        const floorToUse = hasSeenRealData ? lastKnownFloorPrice : BASELINE_PRICE;
  ...
  const processLiveChartData = (
    ...
    const uniquePrices = new Set(minuteData.map(d => parseFloat(d.marketPrice)));
